...
/** 一等 tool 角色结果消息起始的会话格式版本（宿主自 V4 起）。 */
export const NATIVE_TOOL_RESULT_MIN_FORMAT = 4

/**
 * 构造 V4 一等 tool 角色结果消息：`role:'tool'` + 顶层 `toolCallId`/`content`，
 * `isError` 仅在为真时出现。宿主自格式 v4 起 tool/result 的 message 就是这个
 * 形状——没有 `tool-result` 包裹块（V4 恢复边界明确拒绝包裹块：
 * session-format-v3-to-v4 的 assertV4ToolResultMessage）。isError 为假时省略
 * 该字段，与折回 v3 包裹时的字段存在性逐字段对齐（旧线产物不变）。
 * @param input - `{ id, toolCallId, content, isError }`。
 * @returns V4 一等 tool 结果消息。
 */
export function nativeToolResultMessage({ id, toolCallId, content, isError }) {
  return {
    id,
    role: 'tool',
...
/**
 * 把 V4 一等 tool 角色结果消息折回 v3 的 `tool-result` 包裹形状（旧线写盘专用）。
 * @param message - V4 一等 tool 结果消息。
 * @returns v3 形状的消息（`role:'user'` + 恰好一个与 tool source 匹配的包裹块）。
 */
export function wrapperToolResultMessage(message) {
  return {
    id: message.id,
    role: 'user',
    content: [{
...
/**
 * 读出一条 tool/result 消息的语义（读取宽容，两个形状都认）：
 * V4 一等 tool 角色消息取顶层 `toolCallId`/`content`/`isError`；升级前落盘的
 * v3 包裹形状取 `content[0]` 的包裹字段。读取兼容 ≠ 写入旧形状：旧形状只在
 * 目标后端自报格式 < 4 时由 conformToolResultForm 产生。
 * @param message - tool/result 事件的 `data.message`。
 * @returns `{ callId, content, isError }`；两个形状都不是时返回 null。
 */
export function readToolResult(message) {
  ...
  if (typeof message.toolCallId === 'string' && message.toolCallId.length > 0) {
    return { callId: message.toolCallId, content: message.content, isError: message.isError === true }
  }
  ...
  if (block !== null && typeof block === 'object' && block.type === 'tool-result'
    ...
    return { callId: block.toolCallId, content: block.content, isError: block.isError === true }
  }
  return null
}

/**
 * 按目标后端当前会话格式版本统一 tool/result 的消息形状：版本 >= 4 保持 V4
 * 一等 tool 角色消息；版本 < 4 折回 v3 的 `tool-result` 包裹块——v3 及更早的
 * 恢复边界要求 `role:'user'` + 恰好一个与 tool source 匹配的包裹块，直接写
 * V4 形状会被响亮拒绝。版本未知（非数字）时按 V4 处理，与本仓「构造即当前
 * 规范形状」的口径一致。未改动时原样返回调用方数组。
 * @param events - 事件批。
 * @param formatVersion - 目标后端当前会话格式版本。
 * @returns 折回后的事件批。
 */
export function conformToolResultForm(events, formatVersion) {
  if (typeof formatVersion === 'number' && formatVersion >= NATIVE_TOOL_RESULT_MIN_FORMAT) return events
  let changed = false
  const out = events.map((event) => {
    ...
    if (message === null || typeof message !== 'object' || message.role !== 'tool'
      || typeof message.toolCallId !== 'string' || message.toolCallId.length === 0) return event
    changed = true
    return { ...event, data: { ...data, message: wrapperToolResultMessage(message) } }
  })
  ...
  if (t.steps.length === 0) {
    ...
    for (let i = 0; i < t.steps.length; i++) {
      ...
      for (const tc of step.toolCalls) {
        ...
        if (!real) bump('synthesized')
        push('tool/result', {
          turn,
          step: stepNum,
          // 构造即当前规范形状：V4 一等 tool 角色消息。目标后端自报格式 < 4 时
          // 由写盘边界 conformToolResultForm 折回 v3 包裹（旧线产物逐字段不变）。
          message: nativeToolResultMessage({
            id: 'import:' + meta.id + ':t' + turn + ':' + stepNum + ':' + tc.id,
            toolCallId: tc.id,
            content: real ? real.content : [{ type: 'text', text: SYNTHETIC_TOOL_RESULT_TEXT }],
            isError: real ? real.isError === true : true,
          }),
        }, true, [callSeq])
      }
...
export function convertClaudeJsonl(raw, args = {}) {
  ...
  return {
    meta: result.meta,
    events: batches.flat(),
    // 本函数固定 keepTurns: true，故 turns 恒为回合数组（工厂的另一形态是回合数）。
    turns: /** @type {any[]} */ (result.turns),
    title: result.title,
...
export function validateSessionEvents(events, startSeq = 0) {
  ...
  for (const ev of events) {
    ...
    if (ev.type === 'turn/start') {
      ...
      const result = readToolResult(ev.data?.message)
 ...
 */
export function createClaudeStreamConverter(/** @type {{ sessionId?: string, fallbackSessionId?: string, keepTurns?: boolean, skipTurns?: number, startSeq?: number, batchEvents?: number, onBatch?: (batch: readonly unknown[]) => void }} */ {
  ...
  let pending = []
  let carry = ''
  /** @type {{ version: number, id: string, createdAt: number, cwd?: string } | null} */
  let meta = null
  const repaired = { synthesized: 0, duplicateResults: 0, orphanResults: 0 }
