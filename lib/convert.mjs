...
function synthesizeTurnEvents(meta, turn, t, startSeq, provider, mname) {
  const events = []
  let seq = startSeq
...
function synthesizeSession({ meta, turns, title, provider, model, skipped, records, skippedLines, typeCounts }) {
  ...
  for (let turn = 1; turn <= turns.length; turn++) {
    const evs = synthesizeTurnEvents(meta, turn, turns[turn - 1], seq, provider, mname)
    events.push(...evs)
    seq += evs.length
  }
...
export function createClaudeStreamConverter({
  ...
  const finalizeTurn = (t) => {
    ...
    const evs = synthesizeTurnEvents(metaFor(), turnCount, t, nextSeq, 'claude-code', state.model)
    nextSeq += evs.length
