...
/**
 * 读取一个技能文件并解析 frontmatter。README/MEMORY 等非技能文档与缺失
 * name/description 的文件一律跳过（与 skills-provider 一致，issue#1）。
 * @param file - SKILL.md 或扁平 .md 的绝对路径。
 * @returns 技能条目，或 null（不存在/空体/不可读/非技能文档）。
 */
 ...
 * 扁平文件两种形态（Claude Code 布局）；显式排除 README.md/MEMORY.md。
 ...
 */
export async function gitStatus(cwd, /** @type {{ exec?: typeof execFileAsync, timeoutMs?: number, knownBranch?: string }} */ { exec = execFileAsync, timeoutMs = DEFAULT_GIT_TIMEOUT_MS, knownBranch } = {}) {
 ...
 */
export async function scanTranscriptFile(file, /** @type {{ maxBytes?: number, signal?: AbortSignal }} */ { maxBytes = DEFAULT_MAX_TRANSCRIPT_BYTES, signal } = {}) {
...
/**
 * loadImports 的同步变体（供 systemPrompt 同步提供者在提示词组装期读取）。
 * @param cacheDir - resolveCacheDir 的结果。
 * @returns 映射对象，缺失/损坏返回空对象。
 */
export function loadImportsSync(cacheDir) {
  try {
    const parsed = JSON.parse(readFileSync(path.join(cacheDir, 'imports.json'), 'utf8'))
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) return parsed
  } catch {
    // 缺失或损坏：空映射。
  }
