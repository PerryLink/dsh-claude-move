 ...
 */
export async function gitStatus(cwd, { exec = execFileAsync, timeoutMs = DEFAULT_GIT_TIMEOUT_MS } = {}) {
...
export async function scanProjectDir(projectDir, opts = {}) {
  ...
  return {
    ...
    ...((opts.scanGit !== false && dirExists)
      ? { git: await gitStatus(cwd, { exec: opts.gitExec ?? execFileAsync, timeoutMs: opts.gitTimeoutMs ?? DEFAULT_GIT_TIMEOUT_MS }) }
      : {}),
    sessions,
    memories,
