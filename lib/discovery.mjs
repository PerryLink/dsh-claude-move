 ...
 */
export async function gitStatus(cwd, { exec = execFileAsync } = {}) {
  if (!existsSync(path.join(cwd, '.git'))) return null
  try {
    const branchResult = await exec('git', ['-C', cwd, 'rev-parse', '--abbrev-ref', 'HEAD'], {
      timeout: GIT_TIMEOUT_MS, windowsHide: true, maxBuffer: 1 << 20,
    })
    ...
    return { isRepo: true, branch: branchResult.stdout.trim() || null, dirtyCount }
  } catch {
    // git 不可用/超时/非仓库：isRepo 但状态未知。
    return { isRepo: true, branch: null, dirtyCount: null }
  }
}
 ...
 */
export async function scanTranscriptFile(file, { maxBytes = DEFAULT_MAX_TRANSCRIPT_BYTES } = {}) {
  let statResult
  try {
 ...
 * @param opts - `scanFile`/`gitExec`/`scanGit`/`maxBytes`。
 * @returns 项目索引条目。
 */
export async function scanProjectDir(projectDir, opts = {}) {
  ...
  for (const file of files) {
    sessions.push(await scanFile(file, { maxBytes }))
  }
  ...
  return {
    ...
    ...(cwd !== null ? { cwd } : {}),
    dirExists,
    ...((opts.scanGit !== false && dirExists)
      ? { git: await gitStatus(cwd, { exec: opts.gitExec ?? execFileAsync }) }
      : {}),
    sessions,
    memories,
 ...
 *   `scanGit`、`maxBytes`、`excludeProjects`（slug 或子串数组）。
...
export async function scanClaudeHome(claudeHome, opts = {}) {
  ...
  const scanFileCached = async (file, scanOpts) => {
    ...
    if (prev && !prev.error && prev.mtimeMs === st.mtimeMs && prev.sizeBytes === st.size) {
      return prev
    }
  ...
  for (const slug of slugs) {
    ...
    try {
      ...
      project = { slug, dir: projectDir, error: String((err && err.message) || err), sessions: [] }
    }
...
export async function saveImports(cacheDir, imports) {
  await mkdir(cacheDir, { recursive: true })
  await writeFile(path.join(cacheDir, 'imports.json'), JSON.stringify(imports), 'utf8')
}
