  ...
  try {
    ...
    for (const s of sessions) {
      ...
      for (const row of msgStmt.all(s.id)) {
        messages++
        try {
          // node:sqlite 的列值是 SQLOutputValue；非字符串（NULL/BLOB/数值）与解析失败同样是跳过。
          const data = typeof row.data === 'string' ? JSON.parse(row.data) : null
 ...
 */
export async function detect(home, /** @type {{ configHome?: string, signal?: AbortSignal }} */ { configHome = locateConfigHome(), signal } = {}) {
