...
//                    读入时 V4 一等 tool 角色消息与升级前的 v3 包裹形状都认）
...
import { readToolResult } from './convert.mjs'
...
/**
 * 从 user/message 的 content 里提取 tool_result 块。升级前的会话可能把工具
 * 结果内联进 user/message 的 content（而非独立的 tool/result 事件），这条折回
 * 路径**只读不写**：读回升级前的持久形态是向后兼容要求，导出仍只产出 Claude
 * 形状的 tool_result 块，DSH 的 `tool-result` 包裹块绝不会被写出去。
 ...
 * @param {string} fields.type - 记录类型（`user`/`assistant`/`custom-title` 等）。
 * @param {unknown} [fields.message] - 消息体；`custom-title` 记录不带该字段。
 * @param {string} [fields.customTitle] - `custom-title` 记录的标题。
 * @param {string} [fields.sessionId] - 会话 id。
 * @param {string} [fields.cwd] - 工作目录（非字符串或空串时省略该字段）。
 * @param {string} [fields.timestamp] - ISO 时间戳（缺省用当前时间）。
 * @returns {Record<string, any>} 记录对象（各字段异构：字符串/null/消息体）。
 */
 ...
 * @param {any[]} [options.events] - DSH 会话事件数组（元素为松散形状的事件对象）。
 * @param {string|null} [options.sessionId] - 目标会话 id。
 * @param {string|null} [options.cwd] - 会话工作目录。
 * @param {string|null} [options.model] - assistant 消息缺省模型名（事件自带 source.model 时优先）。
 * @returns {{ records: Record<string, any>[], lines: string[], title: string|null, counts: object }}：
  ...
  while (i < list.length) {
    ...
    if (type === 'tool/result') {
      ...
      while (i < list.length && list[i]?.type === 'tool/result') {
        const tr = list[i]
        // 读取宽容：V4 一等 tool 角色消息（顶层 toolCallId/content/isError）与升级前
        // 落盘的 v3 包裹形状都认；导出只写 Claude 形状，绝不回写 DSH 的包裹块。
        const result = readToolResult(tr.data?.message)
        if (result) {
          blocks.push({
            type: 'tool_result',
            tool_use_id: result.callId,
            content: mapToolResultContent(result.content),
