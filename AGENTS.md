...
types.d.ts           唯一类型面：把本插件的消息 source kind 合并声明进宿主 MessageSourceMap
...
```sh
npm install   # 安装 peer 依赖（@deepseek-ai/dsh-tools@>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0 || >=0.1.7-0 <0.2.0、schemastery）
npm test      # node --test 跑 test/**/*.test.mjs
npm run typecheck   # checkJs 门（tsconfig.check.json，含 types.d.ts）
npm run check:ci    # 发布线类型门（tsconfig.check.ci.json）
```
...
- **sessionPersistence 双基线（0.4.0 起）**：index.mjs 内置运行时 shim，按 API 形状探测（create 返回值 / 服务成员 / list 元素），绝不按版本号猜测。handle 基线（alpha 线 `0.1.5-alpha.1` 已发布 `@deepseek-ai/dsh-session-persistence` / `-jsonl`；0.4.0 时曾只存在于 checkout，bec6805d6a）上每次 append 后必须 `flush()`（耐久屏障）并成对 `close()`（单写所有权，失败路径 finally 释放），create 前 header 盖上后端当前格式版本（后端自报 generationFormat.currentVersion / 宿主 `@deepseek-ai/dsh-session` 惰性导入，绝无硬编码）并显式补 `isSeeded:false`，缺 model 的 assistant source 回退 provider，两条写盘路径都按后端自报格式版本决定事件形状：>= 2 给合成的 `assistant/message` 补 `stream: []`（恢复边界断言 `Array.isArray(data.stream)`，缺它则日志可写可读但不可续聊；v0/v1 冻结清单不接受该字段，故必须门控），>= 4 让 `tool/result` 保持 v4 一等 `role:'tool'` 消息（顶层 `toolCallId`/`content`/`isError`，无 `tool-result` 包裹块），< 4 经 `conformToolResultForm` 折回 v3 包裹——绝不写出目标后端自己会拒收的形状。已发布旧 API（≤0.1.2-rc.1）路径的 create+append 调用序列与 0.3.x 逐字节一致（header 不动，tool/result 折回 v3 包裹）。handle 路径由 compat workflow 的 alpha profile job（0.1.7-alpha.2）与 test/compat-v3.test.mjs（真实宿主 `Session.fromRestore`）回归，legacy 线仍由 0.1.2-rc.1 profile job 覆盖；本地 checkout 验证脚本 dev/checkout-handle.mjs 不提交。annotateImports 的 cleanStale 只允许在「已持久化 id 集合完整可解析」时执行——列表元素解析不出 header.id 必须响亮抛出，绝不静默清空 imports.json。
- **消息 source 必须是生产者自有 kind（宿主 0.1.7-alpha.1 起）**：笼统的 `{ kind: 'plugin', plugin }` 已从宿主删除，类型层（`MessageSourceMap` 只剩 user|model|tool|'system-prompt'）与落盘行准入层（session-format-v3-to-v4 的 message-sources.ts 拒 `kind === 'plugin'`）双层拦截。本仓在根 `types.d.ts` 合并声明 `'dsh-claude-move'`（与宿主 tool-jobs 同形；该文件同时进 `files` 与两份 tsconfig 的 `include`），injectContext 注入的交接摘要带该 kind。注入消息带 `UserMessage` 标注，退回旧字面量会让 `npm run typecheck` 变红——别把这条标注当装饰。
