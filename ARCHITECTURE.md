...
- 目标：`dsh 0.1.7-alpha.2`（web profile，当前线会话格式 V4）；peerDeps 保持宽 band `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0 || >=0.1.7-0 <0.2.0`（legacy 线仍会挂载，只是写出那条线自己的形状）；纯 ESM 无构建（git/npm/tarball 三种安装均免 `prepare`/`allowBuilds`）。
