...
14. **sessionPersistence 双基线运行时 shim（0.4.0）**：宿主把 sessionPersistence handle 化（create→SessionHandle、服务级 append/readFrom/locate/listSnapshots 移除、list()/stat() 返回快照）——该 seam 自 alpha 线 `0.1.5-alpha.1`（`@deepseek-ai/dsh-session-persistence` / `-jsonl`）已发布，0.4.0 时还只存在于 checkout（bec6805d6a refactor）；legacy 线（≤0.1.2-rc.1）仍是旧 API。index.mjs 内按 API 形状探测（create 返回值是否含 read/append/flush/close、服务是否含 open、list 元素带 header 还是 id），绝不按版本号猜测：handle 路径 append 后必须 `flush()`（耐久屏障）并成对 `close()`（单写所有权，失败路径 finally 释放）；旧路径调用序列与 0.3.x 逐字节一致。lib/ 保持零 DSH 依赖、零改动。handle 路径由 compat workflow 的 alpha profile job 与 test/compat-v3.test.mjs（真实宿主 `Session.fromRestore`）回归，dev/checkout-handle.mjs 仅作本地 checkout 验证（不提交）。
15. **handle 基线 header/事件规范化（0.4.0 checkout 实测）**：checkout 后端给所有产物盖当前格式版本文件名并要求 header.version 等于后端当前 SESSION_FORMAT_VERSION（读回时文件名/头部版本不一致即响亮拒绝）、isSeeded 显式给出（序列化丢弃 undefined）；读取还按当前格式语义校验 assistant/message 的 model source 必须是非空字符串。shim 在 handle 路径 create 前盖版本（来源：后端实例自报的 generationFormat.currentVersion，回退宿主 `@deepseek-ai/dsh-session` 的 SESSION_FORMAT_VERSION 惰性导入——绝无硬编码）并补 isSeeded:false，缺 model 的 assistant source 回退 provider 字符串；自 0.4.5 起，后端格式版本 >= 2 时给合成的 `assistant/message` 补 `stream: []`——恢复边界 `assertAssistantSettlementShape` 要求 `Array.isArray(data.stream)`，缺字段的日志能 create/append 也能原样读回，但 `Session.fromRestore`（resume/fork/种子会话路径）抛 `seed assistant/message at index N has invalid settlement fields`；v0/v1 的冻结清单不接受该字段，故补 stream 必须按版本门控。自 0.4.8 起同一条版本门控也管 `tool/result` 的消息形状：宿主格式 v4 起它是一等 `role:'tool'` 消息（顶层 `toolCallId`/`content`/`isError`，`content` 里不得再出现 `tool-result` 包裹块），v3 及更早要求 `role:'user'` + 恰好一个与 tool source 匹配的包裹块。lib/convert.mjs 的构造侧只产出 v4 形状，index.mjs 的两条写盘路径按后端自报版本经 conformToolResultForm 折回旧形状——绝不写出目标后端自己会拒收的形状。旧路径 header 不做任何规范化。
16. **消息 source 的生产者自有 kind（0.4.8，宿主 0.1.7-alpha.1）**：宿主删除了笼统的 `{ kind:'plugin', plugin }` 消息来源，两层独立校验兜底——类型层 `MessageSourceMap`（只剩 `user|model|tool|'system-prompt'`，每个生产者在自己的模块里合并声明）与落盘行准入层（session-format-v3-to-v4 的 message-sources.ts 拒 `kind === 'plugin'`，as any 也拦得住）。本仓在 types.d.ts 合并声明 `'dsh-claude-move'`（与宿主 tool-jobs 同形），injectContext 注入的交接摘要带该 kind 且不带 form（载荷是完整交接摘要，不是 notice 所概括的一行账）。类型层由 checkJs 门禁实际把关：注入消息带 UserMessage 标注，退回旧字面量即 typecheck 变红。
...
- 目标：`dsh 0.1.7-alpha.1`（web profile，当前线会话格式 V4）；peerDeps 范围 `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0` 保持不变（本仓保留宽 band：legacy 线仍会挂载，只是写出那条线自己的形状）；纯 ESM 无构建（git/npm/tarball 三种安装均免 `prepare`/`allowBuilds`）。
