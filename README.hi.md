...
- लक्ष्य: `dsh 0.1.0-rc.6` (web प्रोफ़ाइल); peer deps `0.1.0-rc.6` पर स्थिर। Node `^22.19 || >=24`।
- अंतिम सत्यापन **2026-08-13** Windows (Node 22) पर `@deepseek-ai/dsh@0.1.0-rc.6` के विरुद्ध: tarball से नई स्थापना, वास्तविक स्कैन (40 प्रोजेक्ट / 2387 सत्र), वास्तविक बैच आयात 13/13 + आइडेम्पोटेंट पुनः-आयात 13/13, वर्कस्पेस जुड़ाव व पर्सिस्टेंस आर्टिफ़ैक्ट पुष्ट। macOS/Linux बाकी।
- डेवलपर-प्रीव्यू अवधि: संस्करण स्थिर रखें और DSH अपग्रेड के बाद फिर सत्यापित करें।

## अनुमतियाँ और डेटा

- **पढ़ता है** `~/.claude` (ट्रांसक्रिप्ट, यादें, कौशल, CLAUDE.md, settings.json) — पूर्णतः केवल-पठनीय — और आयात के लक्षित प्रोजेक्ट डायरेक्टरी (वर्कस्पेस जुड़ाव)।
- **लिखता है** सार्वजनिक `sessionPersistence` सेवा के ज़रिए DSH सत्र लॉग (केवल-जोड़), वर्कस्पेस-रजिस्ट्री रिकॉर्ड, और अपना कैश `$DSH_HOME/claude-move/` (स्कैन बुकमार्क + आयात मैप)।
- **कभी नहीं** Claude स्रोत फ़ाइलें बदलता, अन्य ऐप्स का डेटा छूता, या नेटवर्क उपयोग करता।
- **कोई क्रेडेंशियल नहीं पढ़ता/भेजता**; ट्रांसक्रिप्ट में संदिग्ध सीक्रेट केवल स्थान से बताए जाते हैं।

## समस्या-निवारण (Troubleshooting)

- पंक्ति प्रभावी नहीं: `dsh --profile <p> --dump-config` में `# == dsh-claude-move` दिखना चाहिए; `dsh plugin --profile <p> add -w ...` फिर चलाएँ।
- web चलता है पर कोई प्रतिक्रिया नहीं: `dsh plugin add` से बने नए प्रोफ़ाइल में सिर्फ़ `dsh-base` होता है — `dsh.profile.bundles` में `@deepseek-ai/dsh-web-app` जोड़ें (मौजूदा `web` प्रोफ़ाइल में कुछ नहीं करना पड़ता)।
- पैनल रूट्स 404: केवल `enableWebPanel: true` और वेब सर्वर के होने पर उपलब्ध; बूट लॉग में FAILED फ़ाइबर देखें।
- आयात में "transcript 过大" विफलता: `maxTranscriptBytes` बढ़ाएँ या उस फ़ाइल को अलग से आयात करें।
- लॉग: बूट विफलताएँ `dsh` कंसोल पर छपती हैं; प्लगइन वर्कस्पेस/आयात-मैप त्रुटियों के लिए `[claude-move]` उपसर्ग से लॉग करता है।
...
MIT — देखें [LICENSE](LICENSE)। तृतीय-पक्ष सूचनाएँ [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) में। सुरक्षा समस्याएँ GitHub Security Advisories (repo Settings → Security) से निजी रूप से रिपोर्ट करें।
