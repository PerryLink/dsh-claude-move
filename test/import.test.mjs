...
test('apply 注册 claude_scan / import_claude / claude_export（及四合一 move_* 工具）', () => {
  const { ctx, registered } = makeCtx({})
  apply(ctx)
  assert.deepEqual(registered.map((d) => d.name), ['claude_scan', 'import_claude', 'claude_export', 'move_detect', 'move_preview', 'move_run'])
})
