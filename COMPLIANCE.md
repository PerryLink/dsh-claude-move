...
| 独立插件包：cordis 是 peerDependency；ESM；git 安装需 `prepare`+`allowBuilds`；发布带构建产物 | cordis/dsh-tools/schemastery 均 peer（cordis `^4.0.2`、dsh-tools `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0`、schemastery `^3.18.2`）；纯 ESM 无构建步骤 → git 安装免 `prepare`/`allowBuilds` | ✅ |
